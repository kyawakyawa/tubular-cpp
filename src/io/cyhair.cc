...
bool CyHair::Load(const std::string &filepath) {
  ...
  if (!has_points) {
    RTLOG_ERROR("No point data in CyHair.");
    return false;
  }

  if ((default_segments_ < 1) && (!has_segments)) {
    RTLOG_ERROR("No valid segment information in CyHair.");
    return false;
  }
  ...
  if (has_segments) {
    ...
    if (1 !=
      ...
      RTLOG_ERROR("Failed to read CyHair segments data.");
      fclose(fp);
      return false;
    }
  }
  ...
  if (has_thickness) {
    ...
    for (size_t i = 0; i < n; i++) {
      RTLOG_INFO("[CyHair] thickness[{}] = {}", i, thicknesses_[i]);
    }
  }

  if (has_transparency) {
    RTLOG_INFO("[CyHair] Has transparency.");
    transparencies_.resize(total_points_);
    if (1 != fread(&transparencies_[0], total_points_ * sizeof(float), 1, fp)) {
      RTLOG_ERROR("Failed to read CyHair transparencies data.");
      fclose(fp);
      return false;
    }
  }
...
bool LoadCyHair(const std::string &filepath, const bool is_y_up,
  ...
  if (!ret) {
    RTLOG_ERROR("[CyHair] Failed to load CyHair file [{}]", filepath);
    return false;
  }

  RTLOG_INFO("[CyHair] num strand is {}", cyhair.num_strands_);
  size_t offset = 0;
  ...
  RTLOG_INFO("[CyHair] finish loading cyhair file [{}]", filepath);
  return true;
}
